 ...
 * is being validated. In memory database is used, and data is checked during
 * tests.
 * 
...
public class ServerIT {
	...
	/**
	 * Test of getSendingOptions service. And check of organisations in
	 * database.
	 * 
	 * @throws IOException
	 * @throws DhxException
	...
	/**
	 * Tests whole flow. Starts with the document being sent from
	 * DHX(sendDocument service). The document will be sent to representee with
	 * subsystem.
	 * 
	 * @throws DhxException
	 */
	...
	public void sendDocumentToSpecial() throws DhxException {
		...
		assertEquals("subsystem.511", items.get(0).getTransport().getDecRecipient().get(1).getOrganisationCode());
		assertNotEquals(BigInteger.valueOf(9999), items.get(0).getDecMetadata().getDecId());
		BigInteger docId = items.get(0).getDecMetadata().getDecId();
	...
	/**
	 * Tests whole flow. Starts with the document being sent from
	 * DHX(sendDocument service). The document from DHX will be faulty, expect
	 * that it will not be received and receiveDocumentswill not give any
	 * documents.
	 * 
	 * @throws DhxException
	 */
	...
	public void sendDocumentFailed() throws DhxException {
		...
		Source envelope = IntegrationTestHelper.getEnvelope(client, service, null, new SendDocument());
		mockClient.sendRequest(RequestCreators.withSoapEnvelope(envelope)).andExpect(ResponseMatchers
				.xpath("//ns9:sendDocumentResponse[1]/ns9:receiptId[1]", getDhxNamespaceMap()).doesNotExist());
