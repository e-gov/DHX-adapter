...
/**
 * Class that gets SOAP request objects(from DVK services), processes requests
 * and gives SOAP response objects.
 * 
...
public class SoapService {
	...
	/**
	 * Method processes sendDocuments request and saves the document into the
	 * database for further sending to DHX service of the recipient.
	 * 
	 * @param documents
	 *            - SOAP request object
	 * @param sender
	 *            - sender of the request(from SOAP header)
	 * @param recipient
	 *            - recipient of the request(from SOAP header)
	...
	public SendDocumentsResponse sendDocuments(SendDocuments documents, InternalXroadMember sender,
		...
		List<Object> containers = capsuleService.getContainersList(documents.getKeha().getDokumendid().getHref(),
				version);
	...
	/**
	 * Method finds not sent documents that are meant to be sent to DHX(outgoing
	 * documents) and sends them using asynchronous DHX sender.
	 */
	@Loggable
	public void sendDocumentsToDhx() {
		...
		List<Recipient> recipients = recipientRepository
				.findByStatusIdAndTransportDokumentOutgoingDocumentAndDhxInternalConsignmentIdNull(statusId, true);
		for (Recipient recipient : recipients) {
			try {
				...
				try {
					...
					log.debug("Erro occured while searching org. ignoring error and continue!" + ex.getMessage(), ex);
				}
	 ...
	 * Paramters from request are also used to find documents to return.
	 * 
	 * @param request
	 *            - SOAP request object
	 * @param sender
	 ...
	 */
	@Loggable
	public ReceiveDocumentsResponse receiveDocuments(ReceiveDocuments request, InternalXroadMember sender,
			InternalXroadMember recipient) throws DhxException {
			...
			if (request.getKeha().getAllyksuseLyhinimetus() == null) {
				...
				docs = documentRepository
						.findByTransportsRecipientsOrganisationAndTransportsRecipientsStatusIdAndTransportsRecipientsStructuralUnit(
								senderOrg, inprocessStatusId, request.getKeha().getAllyksuseLyhinimetus(), pageable);
			}
		} else {
			if (request.getKeha().getAllyksuseLyhinimetus() == null) {
				docs = documentRepository
				...
				docs = documentRepository
						.findByTransportsRecipientsOrganisationAndTransportsRecipientsStatusIdAndFolderAndTransportsRecipientsStructuralUnit(
								senderOrg, inprocessStatusId, folder, request.getKeha().getAllyksuseLyhinimetus(),
								pageable);
	...
	/**
	 * Method marks documents with status provided in request. Documents which
	 * will be marked are parameters provided in request.
	 * 
	 * @param request
	 *            - SOAP request object
	 * @param senderMember
	 ...
	 */
	@Loggable
	public MarkDocumentsReceivedResponse markDocumentReceived(MarkDocumentsReceivedV3RequestType request,
		...
		Organisation senderOrg = organisationRepository.findByRegistrationCodeAndSubSystem(senderMember.getMemberCode(),
		...
		for (TagasisideType status : request.getDokumendid()) {
			...
			if (!found) {
				throw new DhxException(DhxExceptionEnum.TECHNICAL_ERROR,
						"That document is not sent to recipient organisation. dhlId:" + status.getDhlId().longValue()
								+ " organisation:" + senderOrg.getRegistrationCode());
			}
			if (allSent && !doc.getTransports().get(0).getStatusId().equals(StatusEnum.RECEIVED.getClassificatorId())) {
	...
	/**
	 * Method returns statuses of the documents. Documents are found by the
	 ...
	 */
	@Loggable
	public GetSendStatusResponse getSendStatus(GetSendStatus request, InternalXroadMember senderMember,
		...
		try {
			...
			GetSendStatusV2ResponseTypeUnencoded responseAtt = factory.createGetSendStatusV2ResponseTypeUnencoded();
			for (Document doc : documents) {
				...
				item.setOlek(
						StatusEnum.forClassificatorId(doc.getTransports().get(0).getStatusId()).getClassificatorName());
				for (Recipient recipient : doc.getTransports().get(0).getRecipients()) {
					...
					if (recipient.getFaultCode() != null) {
						ee.ria.dhx.server.types.ee.riik.schemas.dhl.Fault fault = new ee.ria.dhx.server.types.ee.riik.schemas.dhl.Fault();
					...
					if (request.getKeha().isStaatuseAjalugu()) {
						for (StatusHistory recipientHistory : recipient.getStatusHistory()) {
							...
							StatusEnum statusEnum = StatusEnum.forClassificatorId(recipientHistory.getStatusId());
							...
							status.setStaatuseAjaluguId(BigInteger.valueOf(recipientHistory.getStatusHistoryId()));
	...
	/**
	 * Method returns list of organisations that are able to receive the
	 * documents using DHX protocol.
	 * 
	 * @param request
	 *            - SOAP request object
	 * @param senderMember
	 ...
	 */
	@Loggable
	public GetSendingOptionsResponse getSendingOptions(GetSendingOptions request, InternalXroadMember senderMember,
			InternalXroadMember recipientMember) throws DhxException {
